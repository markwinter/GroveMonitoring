# Source: https://github.com/Seeed-Studio/Seeed_Python_SI114X
#!/usr/bin/python3
# -*- coding:utf-8 -*-
import time
...
class grove_si114x:
    ...
    def _ReadByte(self,Reg):
        try:
            with SMBus(1) as bus:
                read_data = bus.read_byte_data(self.addr,Reg)
        except OSError:
    ...
    def _WriteByte(self,Reg,Value):
        try:
            with SMBus(1) as bus:
                bus.write_byte_data(self.addr,Reg,Value)
        except OSError:
            raise OSError("Please check if the I2C device insert in I2C of Base Hat")
    ...
    def _ReadHalfWord(self,Reg):
        try:
            with SMBus(1) as bus:
                block = bus.read_i2c_block_data(self.addr,Reg, 2)
        except OSError:
            raise OSError("Please check if the I2C device insert in I2C of Base Hat")
